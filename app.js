...
function loadSettings() {
  ...
  const theme = localStorage.getItem('bm_theme') || 'light'; // 変更: テーマの設定を取得
  ...
  if (themeSelect) themeSelect.value = theme; // 変更: プルダウンに初期値を設定

  setFontSize(fontSize);
  setLineHeight(lineHeight);
  setTheme(theme); // 変更: 関数名をsetThemeに
}
...
// 変更: setTheme関数として新設（ライト・ダーク・ディムを切り替え）
